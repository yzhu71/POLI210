...
```{r Simpson's Paradox}
# Simulating Simpson's Paradox in R
...
# Create a linear relationship within each group
sim_data$y[sim_data$group == 'A'] <- sim_data$y[sim_data$group == 'A'] + sim_data$x[sim_data$group == 'A']
sim_data$y[sim_data$group == 'B'] <- sim_data$y[sim_data$group == 'B'] - sim_data$x[sim_data$group == 'B']
...
# Plot individual data points and aggregated means
p1 <- ggplot(sim_data, aes(x=x, y=y, color=group)) +
  geom_point() +
...
p2 <- ggplot(agg_data, aes(x=mean_x, y=mean_y, color=group)) +
  geom_point() +
  geom_smooth(method='lm', se=FALSE) +
